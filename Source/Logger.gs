const ChannelSkipReason = Object.freeze({
  ...
  LIST_UNAVAILABLE: "Video list could not be retrieved",
...
const logger = (function() {
  ...
  function logNewChannelDiscovery() {
    console.info("    → New channel detected - only latest movie will be added");
  }
  ...
  function logVideoAddition(videoTitle) {
    console.info(`      → Successfully added: ${videoTitle}`);
  }

  function logVideoAdditionFailure(videoTitle, reason) {
    console.warn(`      → Failed to add: ${videoTitle}`);
    console.warn(`        → Reason: ${reason}`);
  }
